...
public class RegisterDTO {
    ...
    @Pattern(regexp = "(?=^[A-Za-z])(?=.+\\d)[A-Za-z\\d]{5,12}", message = "아이디는 영대소문자, 숫자를 이용해 5~12 자리 사이로 작성해야 합니다.")
    private String id;
    
    // @Length(min=4, max=8, message = "비밀번호는 4~8 자리 사이로 작성해야 합니다.")
    // @NotEmpty(message = "비밀번호는 4~8 자리 사이로 작성해야 합니다.")
    @Pattern(regexp = "(?=^[A-Za-z])(?=.+\\d)(?=.*[#$%@!])[A-Za-z\\d#$%@!]{5,12}", message = "비밀번호는 영대소문자, 숫자, 특수문자를 이용해 5~12 자리 사이로 작성해야 합니다.")
