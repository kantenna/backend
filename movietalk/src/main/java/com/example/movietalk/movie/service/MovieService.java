...
public class MovieService {
    ...
    public MovieDTO getRow(Long mno){
        ...
        List<MovieImage> movieImages = result.stream().map(en -> (MovieImage)en[1]).filter(Objects::nonNull).collect(Collectors.toList());
    ...
    //     Map<String, Object> entitMap = dtoToEntity(dto);
    //     // 영화 정보 저장 => 
    //     Movie movie = (Movie)entitMap.get("movie");
    //     movieRepository.save(movie);

    //     // 영화 이미지 저장
    //     List<MovieImage> imgList = (List<MovieImage>)entitMap.get("imgList");
    //     imgList.forEach(img ->{
    //         movieImageRepository.save(img);
    //     });
    ...
    public PageResultDTO<MovieDTO> getMovieList(PageRequestDTO pageRequestDTO){
        ...
        // result.forEach(obj -> {
        //     MovieDTO dto = entityToDto((Movie)obj[0], List.of((MovieImage)obj[1]), (Long)obj[2], (Double)obj[3]);
        //     dtoList.add(dto);
        // });
        ...
        return PageResultDTO.<MovieDTO>withAll().dtoList(dtoList).totalCount(totalCount).pageRequestDTO(pageRequestDTO).build();
    }
