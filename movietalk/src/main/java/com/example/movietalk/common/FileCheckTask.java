...
public class FileCheckTask {
    ...
    public void checkFile(){
        ...
        // 데이터베이스에서 가져온 파일 내용을 path 객체로 수집 (파일 제거 시 원본 파일 + s_)
        List<Path> fileList = movieImageDTOs.stream().map(dto -> Paths.get(uploadPath, dto.getPath(), dto.getUuid() + "_" + dto.getImgName())).collect(Collectors.toList());

        movieImageDTOs.stream().map(dto -> Paths.get(uploadPath, dto.getPath(),  "s_" + dto.getUuid() + "_" + dto.getImgName())).forEach(p -> fileList.add(p));
