...
public class ClubOauth2Service extends DefaultOAuth2UserService{
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 1. google이 넘겨준 정보에서 이메일 추출
        String email = null;
        if (clientName.equals("Google")) {
            email = oAuth2User.getAttribute("email");
        }

        // 2. 테이블에 동일한 이메일이 있는지 확인하고 업ㄱ는 경우 회원가입 / 존재하는 경우 Member 가져오기
        Member member = saveSocialMember(email);

        // 3. MemberDTO 로 변경 후 리턴
        MemberDTO dto = new MemberDTO(member.getEmail(), member.getPassword(), member.isFromSocial(), 
            member.getRoles().stream().map(role -> new SimpleGrantedAuthority("ROLE_"+role.name())).collect(Collectors.toSet()),
    ...
    private Member saveSocialMember(String email){
        ...
        .email(email).name(email).password(passwordEncoder.encode("1111")).fromSocial(true).build();
